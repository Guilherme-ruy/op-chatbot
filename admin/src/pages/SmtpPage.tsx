...
import { Eye, EyeOff, Save, Send, CheckCircle2, XCircle, Mail } from 'lucide-react'
...
export default function SmtpPage() {
  ...
  async function handleSave() {
    ...
    try {
      const payload: SmtpSettingsInput = {
        ...
        ...(form.pass ? { pass: form.pass } : {}),  // só envia se preenchida
      }
      const updated = await saveSmtpSettings(payload)
      setSettings(updated)
      setForm(f => ({ ...f, pass: '' }))  // limpa senha após salvar
      setSaved(true)
  ...
  async function handleTest() {
    ...
    try {
      ...
      setTestResult({
        ok: true,
        message: `E-mail de teste enviado para ${settings?.notification_email}.`,
      })
    } catch (e: any) {
      setTestResult({
        ok: false,
        message: e?.response?.data?.error ?? 'Falha ao enviar e-mail de teste.',
      })
    } finally {
      setTesting(false)
    }
    ...
    <div className="space-y-6 max-w-xl">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Servidor usado para envio de notificações de leads
          </p>
        </div>
        <Badge variant={isConfigured ? 'success' : 'secondary'} className="mt-1 flex-shrink-0">
      ...
      {!isConfigured && (
        <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-xs text-blue-800 flex gap-2">
          ...
          <span>
            Configure o servidor SMTP para que leads qualificados gerem notificações por e-mail
            automaticamente. Funciona com Gmail, Outlook, Sendgrid, Resend e qualquer provedor
            SMTP padrão.
          </span>
        </div>
      )}
